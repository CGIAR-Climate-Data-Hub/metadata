...
| version chain (derived) | Encoder output only | On superseded records: Version Extension `deprecated: true`, `links[rel=successor-version]`, `links[rel=latest-version]` - derived from the `previous_version` graph, not author-facing | Same links on superseded records                                                                      |
